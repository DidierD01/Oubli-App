...
RUN sed -i 's|AllowOverride None|AllowOverride All|g' /etc/apache-config.conf

# Ajoute le nom du serveur pour éviter un avertissement Apache
RUN echo "ServerName metro.proxy.rlwy.net" >> /etc/apache-config.conf
...
RUN mkdir -p /var/log/apache-config/ && \
    touch /var/log/apache-config/access.log /var/log/apache-config/error.log && \
    chown www-data:www-data /var/log/apache-config/access.log /var/log/apache-config/error.log
